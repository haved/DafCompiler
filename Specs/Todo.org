...
**** Array access
*** Scopes
**** TODO Complain when finalOutExpression is thrown strait to a void
**** TODO Require semicolon after scope if returns an expression, unless itself is returned
Already implemented a long time ago, but make the warning better!
This will require a file id system for the TextRanges
**** TODO Scopes w/o finalOutExpression can't be finalOutExpressions
*** DONE With
